...
const questions = [
    {
        id: "directive",
        title: "Explorer Archetype",
        question: "Select your primary operational profile for this deployment.",
        options: [
            { id: "history", label: "Heritage Archivist", sub: "Deep immersion into temples, tombs, and ancient logic.", icon: <Landmark /> },
            { id: "nature", label: "Thrill Strategist", sub: "High-intensity desert treks and Red Sea tactical dives.", icon: <Palmtree /> },
            { id: "luxury", label: "Sovereign Nomad", sub: "Elite relaxation at curated retreats and private villas.", icon: <Wind /> }
        ]
    },
    {
        id: "density",
        title: "Social Payload",
        question: "Who is registered on your travel manifest?",
        options: [
            { id: "low", label: "Solo Operative", sub: "Maximum mobility and stealth for a single traveler.", icon: <Users /> },
            { id: "high", label: "Strategic Delegation", sub: "Coordinated logistics for family or high-level groups.", icon: <Globe /> }
        ]
    },
    {
        id: "tempo",
        title: "Operational Tempo",
        question: "What is your targeted daily immersion intensity?",
        options: [
            { id: "active", label: "Strategic Pulse", sub: "Back-to-back site visits and logistical transitions.", icon: <Activity /> },
            { id: "passive", label: "Deep Archive", sub: "Deliberate, slow-form exploration of single locations.", icon: <Sun /> }
        ]
    },
    {
        id: "terrain",
        title: "Geographical Sector",
        question: "Which Egyptian biome is your primary target?",
        options: [
            { id: "maritime", label: "Nile Valley", sub: "The historical heartline between Cairo and Aswan.", icon: <Waves /> },
            { id: "desert", label: "Great Sand Sea", sub: "Western oases and the silence of the Sahara.", icon: <Tree /> },
            { id: "urban", label: "Red Sea Hub", sub: "Modern coastal infrastructure and marine sectors.", icon: <Building2 /> }
        ]
    }
];
...
export default function AIGenerator() {
    ...
    return (
        <div className="min-h-screen pt-40 pb-20 px-4 overflow-hidden relative">
                ...
                <AnimatePresence mode="wait">
                    {step === 0 && (
                        ...
                        >
                            ...
                            <h1 className="text-7xl md:text-9xl font-black text-white tracking-tighter uppercase leading-[0.8] mb-10">
                                Find Your <br /><span className="gold-gradient">Perfect Trip.</span>
                            </h1>
                            <p className="text-zinc-400 text-lg md:text-xl font-medium max-w-3xl mx-auto leading-relaxed">
                                Our AI calculates your preferences to recommend the best locations in Egypt for your next adventure.
                            </p>
                            <button
                            ...
                            >
                                <span className="relative z-10 flex items-center gap-3">
                                    Start AI Planner <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                                </span>
                                <motion.div
                    ...
                    {step >= 1 && step <= 4 && (
                        ...
                        >
                            <div className="flex justify-between items-end border-b border-white/10 pb-6">
                                <div>
                                    <div className="text-gold text-[10px] font-black uppercase tracking-[0.4em] mb-2">Step 0{step}</div>
                                    <h2 className="text-4xl font-black text-white tracking-tighter uppercase">{questions[step - 1].title}</h2>
                                </div>
                                <div className="text-zinc-800 font-black text-7xl opacity-50 tracking-tighter">{step}/{questions.length}</div>
                            </div>

                            <p className="text-2xl text-white font-medium border-l-4 border-gold pl-6 py-2 bg-gold/5">
                    ...
                    {step === 5 && (
                        ...
                        >
                            ...
                            <div className="space-y-6 max-w-lg mx-auto">
                                <h2 className="text-4xl font-black text-white tracking-tighter uppercase">Creating Your Plan.</h2>
                                <div className="bg-white/[0.02] border border-white/5 p-8 rounded-3xl space-y-3 h-40 overflow-hidden text-left font-mono text-[10px] text-zinc-400">
                                    {logs.map((log, i) => (
                                        <motion.div
                                            key={i}
                                            initial={{ opacity: 0, x: -10 }}
                                            animate={{ opacity: 1, x: 0 }}
                                            className={cn(log?.includes('[SUCCESS]') ? "text-gold" : "")}
                                        >
                                            {log}
                                        </motion.div>
                                    ))}
                    ...
                    {step === 6 && (
                        ...
                        >
                            <div className="text-center space-y-4">
                                <div className="text-gold text-xs font-black uppercase tracking-[0.5em]">Your AI Recommendations</div>
                                <h2 className="text-6xl font-black text-white tracking-tighter uppercase leading-tight">Your Recommended <br /> destinations <span className="gold-gradient">in Egypt.</span></h2>
                            </div>
                                ...
                                {results.map((result, idx) => (
                                    ...
                                    >
                                        <div className="aspect-[16/10] overflow-hidden relative">
                                            <SafeImage priority={idx === 0} src={result.image} alt={result.name} className="w-full h-full group-hover:scale-110 transition-transform duration-[2s]" />
                                            ...
                                            <div className="absolute bottom-8 left-8 right-8">
                                                ...
                                                <h3 className="text-5xl font-black text-white tracking-tighter uppercase">{result.name}</h3>
                                            </div>
                                        </div>
                                        ...
                                        <div className="p-10 space-y-10">
                                            ...
                                            <div className="grid grid-cols-2 gap-8">
                                                <div className="space-y-6">
                                                    <h4 className="text-white text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-3">
                                                        <MapPin className="w-4 h-4 text-gold" /> Famous Places
                                                    </h4>
                                                ...
                                                <div className="space-y-6">
                                                    ...
                                                    <div className="flex flex-col gap-2">
                                                        {result.activities.map((act: string) => (
                                                            <div key={act} className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 flex items-center gap-2 italic">
                                            ...
                                            >
                                                Start Building This Plan <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                                            </button>
                                        </div>
                                    </motion.div>
                                ))}
            ...
            <div className="fixed bottom-0 left-0 p-12 opacity-10 pointer-events-none hidden lg:block">
                <div className="text-[180px] font-black uppercase leading-[0.7] text-white tracking-tighter">TRAVEL.<br />EGYPT.</div>
            </div>
        </div>
    );
}
