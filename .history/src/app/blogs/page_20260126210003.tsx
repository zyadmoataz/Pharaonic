...
const blogs = [
    ...
    },
    {
        ...
        title: "Sovereign Shopping: The Khan el-Khalili Disintermediation",
        excerpt: "How to navigate Egypt's oldest market without the 500% 'hustle tax' using Guidly's authorized benchmarks.",
        date: "Jan 25, 2026",
        author: "Economic Audit Team",
        category: "Economy",
        image: "https://images.unsplash.com/photo-1572252009286-268acec5ca0a?auto=format&fit=crop&q=80&w=800",
        readTime: "9 min"
    },
                ...
                <div className="space-y-12">
                    ...
                    <div className="relative overflow-hidden group py-10">
                        <div className="flex items-center justify-between mb-8">
                            <h4 className="flex items-center gap-3 text-white font-black uppercase tracking-[0.3em] text-[10px]">
                                <Sparkles className="w-4 h-4 text-gold" /> Priority Intelligence
                            </h4>
                        ...
                        >
                            {blogs.slice(0, 3).map((blog, i) => (
                                <Link
                                    ...
                                    key={`feat-${blog.slug}`}
                                    className="min-w-[400px] md:min-w-[600px] aspect-[16/9] rounded-[3rem] overflow-hidden relative border border-white/10 group/feat"
                                >
                                    <img src={blog.image} className="w-full h-full object-cover transition-transform group-hover/feat:scale-110 duration-1000" />
                                    <div className="absolute inset-x-0 bottom-0 p-10 bg-gradient-to-t from-black via-black/40 to-transparent">
                                        <div className="flex items-center gap-4 text-gold text-[10px] font-black uppercase tracking-[0.3em] mb-4">
                                            <span>Most Viewed</span>
                                            <div className="w-px h-3 bg-white/20" />
                                            <span>{blog.category}</span>
                                        </div>
                                        <h3 className="text-3xl font-black text-white tracking-tighter uppercase italic leading-[0.9]">{blog.title}</h3>
                                    </div>
                                </Link>
                            ))}
                        </motion.div>
                        <div className="absolute top-1/2 left-0 w-20 h-full bg-gradient-to-r from-black to-transparent pointer-none z-10" />
                        <div className="absolute top-1/2 right-0 w-20 h-full bg-gradient-to-l from-black to-transparent pointer-none z-10" />
                    </div>
