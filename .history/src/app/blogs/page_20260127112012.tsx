...
import { Search, Calendar, User, ChevronRight, ArrowRight, BookOpen, Sparkles } from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto space-y-20">
                ...
                <div className="space-y-12">
                    ...
                    <div className="relative group py-10">
                        ...
                        <div
                            id="blog-slider"
                            className="flex gap-10 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-10 px-4 -mx-4"
                        >
                            {blogs.slice(0, 3).map((blog, i) => (
                                ...
                                >
                                    <Link href={`/blogs/${blog.slug}`} className="block w-full h-full">
                                        <SafeImage src={blog.image} className="w-full h-full object-cover transition-transform group-hover/feat:scale-110 duration-1000" />
                ...
                <div className="p-20 rounded-[4rem] bg-gold/5 border border-gold/20 text-center space-y-8 relative overflow-hidden">
                    ...
                    <form className="flex flex-col md:flex-row gap-4 max-w-2xl mx-auto" action="https://formspree.io/f/mqaebrda" method="POST">
                        <input name="email" type="email" required placeholder="strategic@network.ai" className="flex-1 bg-black border border-white/10 rounded-2xl px-10 py-6 text-zinc-100 font-bold outline-none focus:border-gold transition-all" />
                        <button type="submit" className="px-12 py-6 bg-gold text-black rounded-2xl font-black uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-gold/10">Authorize Access</button>
                    </form>
                </div>
            </div>
        </div>
