...
import { Search, Calendar, User, ChevronRight, ArrowRight, BookOpen, Sparkles, ChevronLeft, Shield } from "lucide-react";
...
export default function BlogsPage() {
    ...
    return (
        <div className="min-h-screen pt-40 pb-20 px-4 text-zinc-100">
                    ...
                    {filteredBlogs.map((blog, i) => (
                        <Link key={blog.slug} href={`/blogs/${blog.slug}`} className="group">
                            <motion.div
                                ...
                                className="flex flex-col gap-6 p-8 rounded-[3rem] bg-[#0a0a0a] border border-white/5 group-hover:border-gold/30 transition-all h-full relative overflow-hidden"
                            >
                                <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <ArrowRight className="w-5 h-5 text-gold -rotate-45" />
                                </div>

                                <div className="aspect-[16/10] w-full rounded-[2rem] overflow-hidden relative">
                                    <SafeImage src={blog.image} alt={blog.title} className="w-full h-full group-hover:scale-110 transition-transform duration-700 contrast-125" />
                                    <div className="absolute top-4 left-4 px-3 py-1 rounded-full bg-black/60 backdrop-blur-md border border-white/10 text-gold text-[8px] font-black uppercase tracking-widest">{blog.category} Sector</div>
                                    <div className="absolute bottom-4 left-4 px-3 py-1 rounded-full bg-gold text-black text-[8px] font-black uppercase tracking-widest flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Shield className="w-2 h-2" /> Verified
                                    </div>
                                </div>
                                <div className="space-y-4 flex-1 flex flex-col px-2">
                                    <div className="flex items-center gap-4 text-[9px] font-black text-zinc-600 uppercase tracking-[0.2em]">
                                        <span>{blog.date}</span>
                                        <div className="w-1 h-1 rounded-full bg-zinc-800" />
                                        <span>{blog.readTime} Analysis</span>
                                    </div>
                                    <h2 className="text-xl font-black text-white tracking-tighter leading-tight group-hover:text-gold transition-colors uppercase italic">{blog.title}</h2>
                                    <p className="text-zinc-500 text-sm leading-relaxed font-semibold italic flex-1 line-clamp-3">"{blog.excerpt}"</p>
                                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                                        <div className="text-[10px] text-zinc-500 font-bold uppercase">{blog.author}</div>
                                        <div className="text-gold text-[9px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Open Archive</div>
                                    </div>
                                </div>
                            </motion.div>
                        </Link>
