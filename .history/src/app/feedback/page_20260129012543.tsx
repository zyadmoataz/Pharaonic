...
import { Send, CheckCircle, Send as SendIcon } from "lucide-react";
import { useForm, ValidationError } from "@formspree/react";
import { cn } from "@/lib/utils";

const FeedbackPage = () => {
    const [state, handleSubmit] = useForm("mpqdlklz");
                    ...
                    {state.succeeded ? (
                        ...
                        >
                            ...
                            <div className="space-y-6">
                                <h2 className="text-5xl md:text-7xl font-bold text-white tracking-tighter">Thank You!</h2>
                        ...
                        >
                            <div className="mb-12">
                                ...
                                <p className="text-zinc-400 text-lg">Help us architect a better future for Egyptian tourism.</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-12">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-8 rounded-3xl bg-white/[0.02] border border-white/5">
                                    ...
                                    <div className="space-y-2">
                                        <label htmlFor="fb-name" className="text-xs text-zinc-400 uppercase tracking-widest ml-1">Name</label>
                                        <input id="fb-name" name="name" required placeholder="Full Name" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-gold/50 outline-none" value={answers.name} onChange={e => setAnswers({ ...answers, name: e.target.value })} />
                                    </div>
                                    <div className="space-y-2">
                                        <label htmlFor="fb-age" className="text-xs text-zinc-400 uppercase tracking-widest ml-1">Age</label>
                                        <input id="fb-age" name="age" type="number" required placeholder="Age" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-gold/50 outline-none" value={answers.age} onChange={e => setAnswers({ ...answers, age: e.target.value })} />
                                    </div>
                                    <div className="space-y-2">
                                        <label htmlFor="fb-country" className="text-xs text-zinc-400 uppercase tracking-widest ml-1">Country</label>
                                        <input id="fb-country" name="country" required placeholder="Country of Residence" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-gold/50 outline-none" value={answers.country} onChange={e => setAnswers({ ...answers, country: e.target.value })} />
                                    </div>
                                    <div className="space-y-2">
                                        <label htmlFor="fb-stay" className="text-xs text-zinc-400 uppercase tracking-widest ml-1">Duration of Stay</label>
                                        <input id="fb-stay" name="duration" required placeholder="e.g. 10 Days" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-gold/50 outline-none" value={answers.duration} onChange={e => setAnswers({ ...answers, duration: e.target.value })} />
                                    </div>
                                    <div className="col-span-1 md:col-span-2 space-y-2">
                                        <label htmlFor="fb-cities" className="text-xs text-zinc-400 uppercase tracking-widest ml-1">Cities Visited</label>
                                        <input id="fb-cities" name="cities" required placeholder="e.g. Cairo, Luxor, Aswan" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-gold/50 outline-none" value={answers.cities} onChange={e => setAnswers({ ...answers, cities: e.target.value })} />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-10">
                                    ...
                                    {questions.map((q, idx) => (
                                        ...
                                        >
                                            ...
                                            {/* Hidden input to pass value to Formspree */}
                                            <input type="hidden" name={q.key} value={answers[q.key as keyof typeof answers]} />
                                ...
                                >
                                    {state.submitting ? (
                                        ...
                                        <>
                                            Transmit Intelligence <SendIcon className="inline-block ml-3 w-6 h-6 group-hover:translate-x-1 transition-transform" />
                                        </>
                                    )}
                                </button>
                                {state.errors && (
                                    <p className="text-red-500 font-bold text-center mt-4">
                                        Transmission failed. Please verify connection to the Strategic Layer.
                                    </p>
                                )}
                            </form>
                        </motion.div>
