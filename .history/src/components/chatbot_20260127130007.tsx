...
export function Chatbot() {
    ...
    const [messages, setMessages] = useState([
        { role: "bot", content: "I am the NileVista Protocol Liaison. Select a sector below for an authorized briefing, or state your specific inquiry." }
    ]);
    ...
    const knowledgeBase = [
        {
            ...
            a: "NileVista is a boutique tourism infrastructure startup. We specialize in authorized disintermediation—removing the inefficient legacy layers of Egyptian tourism through proprietary AI planning and secure financial protocols."
        },
        {
            q: ["card", "membership", "price", "tier", "gold", "platinum", "elite"],
            a: "The NileVista Card is our proprietary security layer. Tiers: Gold (Free Entry), Platinum ($49/Security Focus), and Elite ($199/Full Operational Authorization). It scrubs your financial metadata in local markets and provides access to fast-track logistics."
        },
        {
            q: ["planner", "ai", "architect", "custom"],
            a: "Our AI Architect (/ai-planner) utilizes a multi-modal logic engine. It calculates duration-based costs, authorizes sector-specific liaisons, and supports Executive Car, Tactical Bus, and Sovereign Air protocols. It's designed to build a surgical blueprint for your deployment."
        },
        {
    ...
    const handleSend = async (e?: React.FormEvent, overridePrompt?: string) => {
        ...
        // 2. Complex Query Trigger
        const isComplex = userQuery.length > 60 || userQuery.includes("person") || userQuery.includes("human") || userQuery.includes("booking issue");
        ...
        if (isComplex) {
            ...
            try {
                ...
                setMessages(prev => [...prev, { role: "bot", content: "Connection interrupted. Please stick to protocol queries or use /contact." }]);
            } finally {
                setIsSending(false);
            }
        } else {
            ...
            setTimeout(() => {
                setMessages(prev => [...prev, {
                    role: "bot",
                    content: "Access denied. My logic core is limited to NileVista protocols (Planning, Permits, Security). Please refine your query or select a sector topic."
                }]);
            }, 800);
        }
    };
