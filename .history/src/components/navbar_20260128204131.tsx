...
const navItems: NavItem[] = [
    { name: "Home", href: "/" },
    {
        ...
        href: "/services",
        dropdown: [
            { name: "Curated Expeditions", href: "/recommendations", description: "Vetted premium tour plans designed for maximum efficiency.", icon: <Sparkles className="w-4 h-4" />, category: "Experience" },
            { name: "Personalized Plan", href: "/personalized-plan", description: "Protocol-driven custom itinerary architecture.", icon: <Zap className="w-4 h-4" />, category: "Intelligence" },
            { name: "AI Persona Discovery", href: "/ai-generator", description: "Strategic identification of your travel personality.", icon: <Sparkles className="w-4 h-4" />, category: "Intelligence" },
            { name: "Pharaonic Card", href: "/card", description: "Sovereign financial security layer and cashless transit.", icon: <Shield className="w-4 h-4" />, category: "Logistics" },
            { name: "Official Permits", href: "/licenses", description: "Authorized licenses for media, drones, and research.", icon: <Globe className="w-4 h-4" />, category: "Intelligence" },
            { name: "Liaison Nexus", href: "/tour-guide", description: "Direct-to-source official government connections.", icon: <User className="w-4 h-4" />, category: "Logistics" },
            { name: "Why Pharaonic", href: "/why-us", description: "Our competitive advantages and systemic parity.", icon: <ArrowRight className="w-4 h-4" />, category: "Support" },
        ]
    },
        ...
        <nav className={cn(
            ...
            <div className="flex justify-between items-center h-12 md:h-14">
                    ...
                    {navItems.map((item) => (
                        ...
                        >
                            <Link
                                href={item.href}
                                className={cn(
                                    "text-[9px] xl:text-[10px] font-bold uppercase tracking-[0.25em] transition-all flex items-center gap-1.5 py-4 relative group/link",
                                    isActive(item.href) || (item.dropdown && item.dropdown.some(d => isActive(d.href)))
                                        ? "text-white" : "text-zinc-500 hover:text-zinc-200"
                                )}
                            >
                                {item.name}
                                {item.dropdown && <ChevronDown className={cn("w-3 h-3 transition-transform duration-500 opacity-50", activeDropdown === item.name && "rotate-180 opacity-100")} />}
                            ...
                            <AnimatePresence>
                                {item.dropdown && activeDropdown === item.name && (
                                    <motion.div
                                        ...
                                        className="absolute top-[100%] left-1/2 -translate-x-1/2 mt-4 w-[850px] bg-zinc-950/95 backdrop-blur-[40px] border border-white/10 rounded-[3rem] p-12 shadow-[0_20px_80px_-20px_rgba(0,0,0,0.8)] z-[60] overflow-hidden"
                                    >
                                            ...
                                            {["Intelligence", "Logistics", "Experience"].map((cat) => (
                                                <div key={cat} className="space-y-8">
                                                    <div className="flex items-center gap-4">
                                                        <h4 className="text-[9px] font-black uppercase tracking-[0.5em] text-gold">{cat}</h4>
                                                        ...
                                                        {item.dropdown?.filter(d => (d as any).category === cat).map((subItem) => (
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex-1">
                                                                    <div className={cn("text-[11px] font-bold mb-1 tracking-tight", isActive(subItem.href) ? "text-gold" : "text-white")}>{subItem.name}</div>
                                                                    <div className="text-[8px] text-zinc-500 font-medium leading-relaxed uppercase tracking-widest">{subItem.description}</div>
                                                                </div>
                                                            </Link>
                                                        ))}
                                                    </div>
                                        ...
                                        <div className="mt-14 pt-10 border-t border-white/5 flex items-center justify-between relative z-10">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <span className="text-[8px] font-bold uppercase tracking-[0.3em] text-zinc-600 italic">Version 2.0.4 - Secure Layer</span>
                                            </div>
                                            <Link href="/services" onClick={() => setActiveDropdown(null)} className="text-[9px] font-black uppercase tracking-[0.3em] text-white hover:text-gold transition-colors flex items-center gap-3 group/all">
                                                Complete Ecosystem <div className="p-1.5 rounded-full bg-white/5 group-hover/all:bg-gold transition-colors"><ArrowRight className="w-3 h-3 group-hover/all:text-black transition-colors" /></div>
                                            </Link>
                                        </div>
                                    </motion.div>
                                )}
                ...
                <div className="hidden lg:flex items-center gap-8">
                    ...
                    <Link
                        href="/contact"
                        className="bg-white text-zinc-950 px-8 py-3.5 rounded-full text-[9px] font-black uppercase tracking-[0.3em] hover:bg-gold transition-all duration-500 shadow-lg shadow-white/5"
                    >
                        Initiate Protocol
                    </Link>
                </div>
