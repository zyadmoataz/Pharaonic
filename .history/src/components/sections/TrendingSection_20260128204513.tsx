            ...
            <div className="max-w-7xl mx-auto px-6 mb-24">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-12">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 flex-1">
                        <p className="text-zinc-500 text-lg md:text-xl font-medium max-w-sm leading-relaxed uppercase tracking-widest italic border-l border-gold/30 pl-10">
                    ...
                    >
                        <div className="relative aspect-[4/5] rounded-[4rem] overflow-hidden border border-white/5 bg-zinc-950 shadow-2xl transition-all duration-700 group-hover:border-gold/30 group-hover:shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)]">
                            ...
                            <div className="absolute top-10 left-10 right-10 flex justify-between items-start z-20">
                                <div className="px-5 py-2 rounded-full bg-zinc-950/60 backdrop-blur-xl border border-white/10 text-gold text-[8px] font-black uppercase tracking-[0.3em]">
                                    {spot.sector}
                                </div>
                                <div className="flex flex-col items-end gap-1">
                                    <div className="flex items-center gap-2 text-gold text-lg font-black uppercase tracking-tighter italic">
                                        <ArrowUpRight className="w-4 h-4" />
                                        {spot.growth}
                                    </div>
                                    <div className="text-zinc-500 text-[8px] font-bold uppercase tracking-[0.4em]">Volume Index</div>
                                </div>
                            </div>

                            <div className="absolute bottom-12 left-12 right-12 space-y-6 z-20">
                                <div className="space-y-3">
                                    <div className="flex items-center gap-2 text-gold text-[9px] font-black uppercase tracking-[0.4em]">
                                        <div className="w-1.5 h-1.5 rounded-full bg-gold animate-pulse" />
                                        {spot.status}
                                    </div>
                                    <h3 className="text-4xl md:text-5xl font-black text-white tracking-tighter uppercase italic leading-[0.9] group-hover:text-gold transition-colors duration-500">
                                        {spot.name}
                                    </h3>
                                </div>
                                <p className="text-zinc-500 text-[10px] md:text-xs font-medium leading-relaxed uppercase tracking-widest max-w-sm transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700">
                                    {spot.description}
                                </p>
                            </div>
