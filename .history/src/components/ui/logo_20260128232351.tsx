...
export const Logo: React.FC<LogoProps> = ({ className, size = "md" }) => {
    ...
    return (
        <div className={cn("flex items-center gap-3 group cursor-pointer", className)}>
            ...
            <div className={cn(size === "sm" ? "text-lg" : size === "md" ? "text-xl" : size === "lg" ? "text-4xl" : "text-5xl") + " font-serif"}>
                <span className={cn(
                    "font-black tracking-[0.1em]  text-white transition-colors duration-500",
                    "font-serif", // We'll map this to Cinzel in CSS
                )}>
                    Pharaonic
                </span>
                <span className="text-gold/80 font- tracking-[0.2em]">Guide</span>
